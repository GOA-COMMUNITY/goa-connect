  ...
  const { data: counts } = useQuery({
    ...
    queryFn: async () => {
      const [followers, following, posts] = await Promise.all([
        ...
        supabase.from("user_shorts").select("id", { count: "exact", head: true }).eq("user_id", user!.id),
      ]);
      return { followers: followers.count ?? 0, following: following.count ?? 0, posts: posts.count ?? 0 };
    },
  });
      ...
      <div className="space-y-4 p-4">
        ...
        {user && (
          <ShortUpload
            userId={user.id}
            area={profile?.area}
            onUploaded={() => qc.invalidateQueries({ queryKey: ["profile-counts", user.id] })}
          />
        )}
